...
export default function ProviderUpload() {
  ...
  const uploadFile = async (
    ...
    file: File,
    dates?: { issued_at?: string; expires_at?: string },
  ) => {
    if (!token) return;
          ...
          <div className="flex items-center gap-3">
            {platform?.logo_url ? (
              ...
              <div
                ...
                style={{
                  backgroundColor: platform?.primary_color || "#0F4C81",
                }}
              >
      ...
      <main className="max-w-2xl mx-auto p-4 pb-24">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {docStatus.status === "rejected" && (
                      <p className="text-xs text-destructive">
                        Document rejeté - veuillez en soumettre un nouveau
                      </p>
                    )}
                    {docStatus.status === "expired" && (
                      <p className="text-xs text-destructive">
                        Document expiré - veuillez en soumettre un nouveau
                      </p>
                    )}
                  </div>
        ...
        <div className="mt-8 text-center">
          ...
          <p className="text-xs text-muted-foreground mt-1">
            Vos documents sont sécurisés et accessibles uniquement par{" "}
            {platform?.name}
          </p>
        </div>
      </main>
        ...
        <div className="fixed inset-0 z-50 bg-black/50 flex items-end sm:items-center justify-center p-4">
          <div className="bg-card rounded-t-2xl sm:rounded-2xl w-full max-w-md p-6 animate-in slide-in-from-bottom">
            ...
            <p className="text-sm text-muted-foreground mb-4">
              Veuillez indiquer les dates figurant sur le document "
              {selectedDoc.name}"
            </p>
