...
> = {
  ...
  verified: { label: "V├®rifi├®", variant: "default" },
...
Cordialement`;
  ...
  const handleSendProviderEmail = () => {
    ...
    toast.success("Brouillon email pr├¬t dans votre client mail.");
  };
  ...
  const handleCreateProvider = async () => {
    ...
    if (!businessName) {
      toast.error("Nom de soci├®t├® requis");
      return;
    }
    ...
    if (error || !data) {
      toast.error("Erreur cr├®ation prestataire");
      setCreating(false);
      return;
    }

    toast.success("Prestataire cr├®├®");
    setCreating(false);

    // G├®n├¿re directement un lien Magic Link
  ...
  const handleGenerateLink = async (providerId: string) => {
    ...
    if (error) {
      toast.error("Erreur g├®n├®ration lien");
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="text-muted-foreground">
              G├®rez vos prestataires et g├®n├®rez des liens d&apos;upload
            </p>
          </div>
              ...
              <table className="w-full text-sm">
                <thead className="bg-muted/50">
                  <tr>
                    ...
                    <th className="text-left p-3 font-medium">T├®l├®phone</th>
        ...
        <Dialog open={showNewModal} onOpenChange={setShowNewModal}>
          <DialogContent className="sm:max-w-[560px]">
            <DialogHeader>
              ...
              <DialogDescription>
                Cr├®ez un prestataire et g├®n├®rez un lien Magic Link pour upload
              </DialogDescription>
            </DialogHeader>
            ...
            <DialogFooter>
              ...
              <Button onClick={handleCreateProvider} disabled={creating}>
                {creating ? "Cr├®ation..." : "Cr├®er + g├®n├®rer lien"}
              </Button>
            </DialogFooter>
          </DialogContent>
        </Dialog>
